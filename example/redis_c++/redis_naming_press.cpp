...
DEFINE_string(connection_type, "single", "连接类型：single（单连接）、pooled（连接池）、short（短连接）");

// Redis连接配置
DEFINE_string(redis_naming_service, "", "Redis命名服务地址，例如：http://xxx.redis:6380（必填）");
DEFINE_string(load_balancer, "rr", "负载均衡算法：rr、random、c_murmurhash、c_md5等");
...
// 测试数据生成配置
DEFINE_string(config_file, "", "包含key-value对的配置文件路径（可选，如果不指定则自动生成数据）");
...
// 测试配置参数
DEFINE_string(test_mode, "get_only", "压测模式：get_only（纯GET）或mixed（GET/SET混合）");
 ...
 * @return 成功返回true
 */
bool GenerateKeyValuePairs(std::vector<KeyValuePair>* kv_pairs, 
                          int count, int key_length, int value_size) {
    ...
    for (int i = 0; i < count; ++i) {
        ...
        // 每生成1000个输出一次进度
        if ((i + 1) % 1000 == 0) {
            LOG(INFO) << "Generated " << (i + 1) << "/" << count << " key-value pairs";
        }
    }
    
    LOG(INFO) << "Successfully generated " << kv_pairs->size() << " key-value pairs";
    return true;
}
 ...
 * - 键和值两端的空白字符会被自动去除
 */
bool LoadKeyValuePairs(const std::string& filename, std::vector<KeyValuePair>* kv_pairs) {
    ...
    while (std::getline(file, line)) {
        ...
        if (pos == std::string::npos) {
            LOG(WARNING) << "Invalid line " << line_num << " in config file: " << line;
            continue;
        }
    ...
    LOG(INFO) << "Loaded " << kv_pairs->size() << " key-value pairs from " << filename;
 ...
 */
bool InitializeKeys(brpc::Channel* channel, const std::vector<KeyValuePair>& kv_pairs) {
    ...
    for (size_t i = 0; i < kv_pairs.size(); i += batch_size) {
        ...
        for (size_t j = i; j < end; ++j) {
            if (!request.AddCommand("SETEX %s %d %s", 
                        kv_pairs[j].key.c_str(), 
                        FLAGS_expire_seconds,
                        kv_pairs[j].value.c_str())) {
                LOG(ERROR) << "Failed to add SETEX command for key: " << kv_pairs[j].key;
                return false;
            }
        }
        ...
        if (cntl.Failed()) {
            LOG(ERROR) << "Failed to initialize keys batch " << i/batch_size << ": " << cntl.ErrorText();
            return false;
        }
...
static void* get_only_sender(void* void_args) {
    ...
    std::uniform_int_distribution<> key_dis(0, args->kv_pairs->size() - 1);    // 选择存在的键
    std::uniform_int_distribution<> miss_dis(0, args->miss_keys->size() - 1);  // 选择不存在的键
        ...
        if (!cntl.Failed()) {
            ...
            if (response.reply_size() > 0) {
                if (response.reply(0).is_nil()) {
                    ...
                    // Redis返回错误
                    g_get_error_count << 1;
                    LOG_IF(ERROR, !FLAGS_dont_fail) << "GET error: " << response.reply(0).error_message();
                }
            }
        } else {
            // 请求失败（网络错误、超时等）
...
static void* mixed_sender(void* void_args) {
    ...
    std::uniform_int_distribution<> key_dis(0, args->kv_pairs->size() - 1);    // 选择键
    std::uniform_int_distribution<> miss_dis(0, args->miss_keys->size() - 1);  // 选择未命中键
        ...
        if (is_set_op && !args->kv_pairs->empty()) {
            ...
            if (!cntl.Failed()) {
                ...
                if (response.reply_size() > 0 && !response.reply(0).is_error()) {
                    ...
                    g_set_error_count << 1;
                    LOG_IF(ERROR, !FLAGS_dont_fail) << "SETEX error: " << response.reply(0).error_message();
                }
            } else {
                // SET请求失败
                g_set_error_count << 1;
...
int main(int argc, char* argv[]) {
    ...
    if (!FLAGS_config_file.empty()) {
        ...
        // 自动生成测试数据
        if (!GenerateKeyValuePairs(&kv_pairs, FLAGS_key_count, FLAGS_key_length, FLAGS_value_size)) {
    ...
    // 初始化连接到命名服务
    if (channel.Init(FLAGS_redis_naming_service.c_str(), FLAGS_load_balancer.c_str(), &options) != 0) {
        LOG(ERROR) << "Failed to initialize channel to " << FLAGS_redis_naming_service;
        return -1;
    }

    LOG(INFO) << "Successfully connected to redis naming service: " << FLAGS_redis_naming_service;
    ...
    // 根据测试模式选择工作函数
    void* (*sender_func)(void*) = (FLAGS_test_mode == "get_only") ? get_only_sender : mixed_sender;
    ...
    for (int i = 0; i < FLAGS_thread_num; ++i) {
        ...
        if (!FLAGS_use_bthread) {
            ...
            // 使用bthread
            if (bthread_start_background(&bids[i], NULL, sender_func, &args[i]) != 0) {
    ...
    LOG(INFO) << "Started " << FLAGS_thread_num << " " 
              << (FLAGS_use_bthread ? "bthreads" : "pthreads")
              << " in " << FLAGS_test_mode << " mode";
    ...
    while (!brpc::IsAskedToQuit()) {
        ...
        if (FLAGS_test_mode == "get_only") {
            // 纯GET模式的统计输出
            LOG(INFO) << "GET - QPS: " << g_get_latency_recorder.qps(1)
                      ...
                      << ", Hits: " << g_get_hit_count.get_value()
                      << ", Misses: " << g_get_miss_count.get_value()
            ...
            LOG(INFO) << "SET - QPS: " << g_set_latency_recorder.qps(1)
                      ...
                      << ", P99.9: " << g_set_latency_recorder.latency_percentile(0.999)
                      << ", P99.99: " << g_set_latency_recorder.latency_percentile(0.9999)
